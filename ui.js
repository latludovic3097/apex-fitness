...
function rSession(){
  ...
  if(isW){
    ...
    const imgs=ex.imgs?`<div class="ex-imgs">${ex.imgs.map((p,i)=>`<div class="ex-img-wrap"><img src="${I}${p}" alt="${esc(ex.name)} — ${i?'position de fin':'position de départ'}" loading="lazy" onerror="this.parentElement.innerHTML='<div style=padding:20px;text-align:center;font-size:12px;color:var(--mt)>—</div>'"><div class="ex-img-label">${i?'Fin':'Départ'}</div></div>`).join("")}</div>`:"";
    ...
    content=`<div class="card"><div style="font-size:18px;font-weight:900;letter-spacing:3px;color:var(--wa);margin-bottom:4px">WOD — ${wod.type}${wodHeaderDur}</div><div style="font-size:13px;font-weight:700;color:var(--t2);margin-bottom:10px">${wod.name}</div>${wt}<div style="margin-top:14px">${wod.movements.map(m=>`<div class="wod-move">${m.img?`<div class="wod-img"><img src="${m.img}" alt="${esc(m.name)}" loading="lazy" onerror="this.parentElement.style.display='none'"></div>`:''}<div style="flex:1"><div style="font-size:13px">${m.name}</div><div style="display:flex;gap:4px;margin-top:3px"><a href="${wk(m.name)}" target="_blank" style="font-size:10px;color:#4ecdc4;text-decoration:none;background:rgba(78,205,196,.1);padding:2px 6px;border-radius:4px;font-weight:600">Wiki</a>${m.yt?`<a href="${m.yt}" target="_blank" style="font-size:10px;color:#ff0000;text-decoration:none;background:rgba(255,0,0,.08);padding:2px 6px;border-radius:4px;font-weight:600">▶ Vidéo</a>`:``}</div></div></div>`).join("")}</div><div style="margin-top:16px"><div style="font-size:12px;text-transform:uppercase;letter-spacing:1px;color:var(--mt);margin-bottom:6px;font-weight:600">Notes</div><textarea class="inp" id="sN" placeholder="Ressenti, PR..." oninput="S.notes=this.value;saveA()">${esc(S.notes)}</textarea></div><button class="btn btn-ok" style="margin-top:14px" onclick="finish()">✓ Terminer</button></div>`;
  }
...
function rHist(){
  ...
  if(!S.hist.length)return`<div class="hdr"><div class="page-title">Historique</div></div>${charts}<div class="card" style="text-align:center;color:var(--mt);padding:30px">Aucune séance 💪</div>`;
  return`<div class="hdr"><div class="page-title">Historique</div></div><div style="padding:0 14px 8px;display:flex;gap:8px"><button class="btn2" style="flex:1" onclick="exportCSV()">📊 CSV (Excel)</button><button class="btn2" style="flex:1" onclick="shareLastWorkout()">📸 Partager</button></div>${rCalendarHeatmap()}${rAchievementsCard()}${charts}`+
...
function rSett(){
  return`<div class="hdr"><div class="page-title">Réglages</div></div>
